...
<?php if ($themify->page_title != 'yes'): ?>
	<div class="page-category-title-wrap">
		...
		<h1 class="page-title" itemprop="name"><?php echo is_page() ? get_the_title() : (is_404() ? __('404', 'themify') : ''); ?></h1>
		<?php $sub_heading = themify_get('page_sub_heading'); ?>
		<?php if ($sub_heading || is_404()): ?>
			<p class="page-category-description"><?php echo is_page() ? $sub_heading : (is_404() ? __('Page not found.', 'themify') : '') ?></p>
		<?php endif; ?>
	</div>
<?php endif; ?>	
<!-- /page-title -->
		...
		<?php if ($themify->query_category != ''): ?>
			...
			<?php if (have_posts()): ?>
				...
				<?php if (themify_is_query_page()) : ?>
					<?php if ($themify->page_navigation != 'yes' || 'infinite' === themify_theme_get('more_posts', 'infinite')): ?>
